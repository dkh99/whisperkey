...
class WhisperKeySettings:
    ...
    def _get_default_base_prompts(self) -> Dict[str, str]:
        ...
        return {
            "base_prompt": (
                "You are an expert editor who specialises in cleaning "
                "up dictated text using British English conventions."
            ),
...
- Structure requests logically for AI assistants"""
            ...
            "slack": {
                "system_prompt": (
                    f"{base_prompt} You format text for Slack messages, "
                    "keeping them concise and team-friendly."
                ),
                "instructions": f"""{base_instructions}
...
- Use appropriate formal vocabulary"""
            ...
            "casual_email": {
                "system_prompt": (
                    f"{base_prompt} You format text for casual emails, "
                    "maintaining a friendly but professional tone."
                ),
                "instructions": f"""{base_instructions}
...
- Balance casualness with clarity"""
            ...
            "casual_message": {
                "system_prompt": (
                    f"{base_prompt} You format text for casual messages, "
                    "keeping them natural and conversational."
                ),
                "instructions": f"""{base_instructions}
...
class SettingsDialog(QDialog):
    ...
    def create_prompts_tab(self) -> QWidget:
        ...
        self.base_prompt_edit.setPlaceholderText(
            "Enter the base system prompt that defines the AI's core "
    ...
    def create_audio_tab(self) -> QWidget:
        ...
        self.deepgram_key_edit.setToolTip(
            "If provided, VoxVibe will use Deepgram for faster "
            "transcription. Leave blank to keep Whisper."
        )
    ...
    def reset_context_prompts(self, context_id: str):
        ...
        if context_id in defaults and context_id in self.prompt_editors:
            ...
            QMessageBox.information(
                self, "Reset Complete",
                f"✅ {context_name} prompts reset to defaults!"
            )
    ...
    def reset_all_prompts(self):
        ...
        reply = QMessageBox.question(
            ...
            "Are you sure you want to reset ALL prompts (including base "
            "prompts) to their defaults?\n\nThis cannot be undone.",
