...
class HotkeyService:
        ...
        with self._lock:
            ...
            if self._pending_stop_timer and self.mode == RecordingMode.HOLD_TO_TALK:
                ...
                if alt_space_restored:
                    print("🔑 Combo restored, canceling pending stop")
        ...
        if self.mode == RecordingMode.HOLD_TO_TALK:
            ...
            if key in self._key_release_times:
                time_since_last_release = (
                    current_time - self._key_release_times[key]
                )
                # Debug: print(
                #     f"🔑 Time since last release of {key}: "
                #     f"{time_since_last_release:.3f}s"
                # )
                if time_since_last_release < self._debounce_delay:
                    # Debug: print(
                    #     f"🔑 DEBOUNCING key release for {key} "
                    #     f"(too soon: {time_since_last_release:.3f}s < "
                    #     f"{self._debounce_delay}s)"
                    # )
                    return
            # else:
    ...
    def _start_hold_to_talk(self):
        ...
        if self._pending_stop_timer:
            print("🔧 Canceling leftover timer from previous recording")
            self._pending_stop_timer.cancel()
            self._pending_stop_timer = None
    ...
    def _schedule_stop_with_grace_period(self):
        ...
        if self._pending_stop_timer:
            print("🔧 Canceling existing pending timer")
            self._pending_stop_timer.cancel()
        ...
        # Debug: print(
        #     f"🔑 Scheduled stop check in {self._combo_grace_period}s "
        #     f"(timer: {id(self._pending_stop_timer)})"
        # )
