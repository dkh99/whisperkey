...
class MicBar(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            print(
                f"🖱️ Mouse pressed on MicBar at local pos: "
                f"({local_x:.1f}, {local_y:.1f})"
            )
            print(
                f"🖱️ Mouse pressed on MicBar at global pos: "
            ...
            start_x = self.drag_start_window_pos.x()
            start_y = self.drag_start_window_pos.y()
            print(
                f"🖱️ Started dragging MicBar from window pos: "
                f"({start_x}, {start_y})"
            )
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.dragging:
            ...
            global_y = event.globalPosition().y()
            print(
                f"🖱️ Mouse released on MicBar at global pos: "
                f"({global_x:.1f}, {global_y:.1f})"
            )
            final_x = final_pos.x()
            final_y = final_pos.y()
            print(
                f"🖱️ Finished dragging MicBar to final pos: "
                f"({final_x}, {final_y})"
            )
