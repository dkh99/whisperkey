...
class LLMProcessor(QObject):
    ...
    def _extract_explicit_context(self, text: str) -> tuple[str, Optional[str]]:
        ...
        context_patterns = {
            'code_window': [
                r"this is (a )?(code|coding|text) (window|editor|message)",
                r"send (this )?as (a )?(code|coding) (window|message)",
                r"context:\s*code"
            ],
            'coding_agent': [
                r"this is (a )?coding agent (request|message)",
                r"send (this )?to (the )?coding agent",
                r"context:\s*coding agent"
            ],
            'slack': [
                r"this is (a )?slack message",
                r"send (this )?as (a )?slack message",
                r"context:\s*slack"
            ],
            'whatsapp': [
                r"this is (a )?whatsapp message",
                r"send (this )?as (a )?whatsapp message",
                r"context:\s*whatsapp"
            ],
            'formal_email': [
                r"this is (a )?formal email",
                r"send (this )?as (a )?formal email",
                r"context:\s*formal email"
            ],
            'casual_email': [
                r"this is (a )?casual email",
                r"send (this )?as (a )?casual email",
                r"context:\s*casual email"
            ],
            'casual_message': [
                r"this is (a )?casual message",
                r"send (this )?as (a )?casual message",
                r"context:\s*casual message"
            ],
        }
    ...
    def _detect_communication_context(self, text: str) -> str:
        ...
        if any(phrase in text_lower for phrase in [
            ...
            'cursor', 'code', 'coding', 'agent', 'programming', 'debug',
            'function', 'variable'
        ]):
            return 'coding_agent'
            ...
            'dear sir', 'dear madam', 'to whom it may concern', 'yours faithfully',
            'yours sincerely'
        ]):
            return 'formal_email'
        elif any(word in text_lower for word in ['email', 'mail']) and any(
            word in text_lower for word in [
                'meeting', 'proposal', 'contract', 'business'
            ]
        ):
            return 'formal_email'
    ...
    def _get_system_prompt(self, context_type: str) -> str:
        ...
        if self.settings:
            ...
            base_prompt = (
                "You are an expert editor who specialises in cleaning up "
                "dictated text using British English conventions."
            )

        context_prompts = {
            'code_window': (
                f"{base_prompt} You format text for direct insertion into "
                "code editors or IDEs, keeping it concise and code-appropriate."
            ),
            'coding_agent': (
                f"{base_prompt} You focus on technical communication with "
                "coding assistants, maintaining technical accuracy whilst "
                "improving clarity."
            ),
            'slack': (
                f"{base_prompt} You format text for Slack messages, keeping "
                "them concise and team-friendly."
            ),
            'whatsapp': (
                f"{base_prompt} You format text for WhatsApp messages, "
                "keeping them casual and conversational."
            ),
            'formal_email': (
                f"{base_prompt} You format text for formal business emails "
                "using proper British English etiquette."
            ),
            'casual_email': (
                f"{base_prompt} You format text for casual emails, maintaining "
                "a friendly but professional tone."
            ),
            'casual_message': (
                f"{base_prompt} You format text for casual messages, keeping "
                "them natural and conversational."
            )
        }
